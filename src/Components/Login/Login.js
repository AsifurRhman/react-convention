...
const Login = () => {
    ...
    if (error || error1 || error2||error3) {
        errorMsg =
            <p className='text-red-700'>Error : {error?.message} {error1?.message} {error2?.message}
            {error3?.message}
            </p>;
    }
    ...
    return (
        <div className='mt-32 mb-10 w-full md:w-1/2 mx-auto custom-shadow bg-[#e8eaec] pt-10 pb-10 px-10 rounded-lg'>
            ...
            <p className=" font-bold mt-2 ">
                ...
                New To  <span className='text-sky-900 sp-style text-xl'>Royal ~ Convention Hall</span> ? <Link className='form-link' to='/signup'><span className='text-red-700'>Create an Account</span></Link>
